...
export function MemberManagementPage() {
  ...
  return <main><h1>家庭成员</h1><p>创建者可在此生成成员账号和首次登录 Token。</p><form onSubmit={submit}><label>账号<input name="username" required /></label><label>初始 Token<input name="token" type="password" minLength={16} required /></label>{message ? <p role="status">{message}</p> : null}<button disabled={busy}>{busy ? '正在创建…' : '创建成员'}</button></form></main>
}
