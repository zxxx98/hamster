...
export function LowStockPanel() {
  ...
  const load = useCallback(async () => { const { supabase } = await import('../../lib/supabase'); const rows = await loadOpenReminders(supabase as never); setItems(rows as unknown as Reminder[]) }, [])
  useEffect(() => { void load().catch(() => undefined); const refresh = () => { void load().catch(() => undefined) }; window.addEventListener('household-data-updated', refresh); return () => window.removeEventListener('household-data-updated', refresh) }, [load])
